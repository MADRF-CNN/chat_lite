...
| `/mask` | `/mk`、`/m`、`/boss` | **极速伪装模式**：瞬间清除屏幕与回滚历史，模拟为 OpenAI Codex 交互终端 |
| `/unmask` | `/umk`、`/um`、`/chat` | **解除伪装**：退出伪装模式，恢复即时聊天界面（亦可输入 `exit` 或 `q`） |
...
### 1. 伪装掩护模式（`/mk` 或 `/mask`）
- **极速进入**：在终端输入 `/mk`（或 `/m`、`/mask`）回车，也可在启动时直接运行 `chat-lite -m`：
  - 硬件级瞬间清空屏幕与终端回滚历史缓冲区（彻底防止翻页偷看），所有聊天内容、群名和联系人瞬间消失；
  ...
  - 输入 `/umk`、`/um`、`exit`、`quit` 或 `q` 即可瞬间无缝返回聊天界面。
